...
public class ActionAddRecipe extends ActionDragonForgeBase {
    ...
    @Override
    public void undo() {
        IafRecipeRegistry.ALL_FORGE_RECIPES.remove(recipe);

        getRecipeList(recipe.getDragonType()).remove(recipe);
    }

    @Override
    public String describeUndo() {
        return String.format("Undoing addition of %s Dragon Forge Recipe, that outputs: %s, with blood: %s and input: %s", recipe.getDragonType(), recipe.getOutput(), IIngredient.fromIngredient(recipe.getBlood()), IIngredient.fromIngredient(recipe.getInput()));
    }
}
